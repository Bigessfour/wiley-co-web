...
| Capital gap / debt coverage panels | `POST /api/workspace/capital-gap` and `POST /api/workspace/debt-coverage` must return HTTP 200 on an empty budget DB: synthetic fallbacks are controlled by `WorkspacePanels:Fallback` (see `WileyCoWeb.Api/appsettings.json`). Optional one-time budget rows: `Database:EnsureWorkspacePanelBudgetWhenEmpty` (also in `update-config.json` for App Runner). Executive summaries containing **(sample data)** mean the synthetic path ran—import real budget data when available. |
| App Runner image refresh           | After API image changes, trigger a new deployment (e.g. `aws apprunner start-deployment --service-arn <arn>` or console **Deploy**). Verify `/health`, then the two POST routes above and `GET /api/workspace/snapshot` return 200 (not 404). PowerShell smoke: [Scripts/verify-apprunner-workspace-api.ps1](../Scripts/verify-apprunner-workspace-api.ps1) (uses FY **2037** so capital gap exercises the empty-FY synthetic path once this branch is deployed).                 |
