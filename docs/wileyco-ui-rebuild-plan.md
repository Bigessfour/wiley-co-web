...
Status: Repo-level web rebuild and the AI centerpiece are functionally far along, but operational production readiness is not complete yet. AWS validation on April 15, 2026 confirmed that App Runner now serves `WileyCoWeb.Api` at the public hostname, `/health` responds successfully, `/api/workspace/snapshot` returns populated enterprise data, `/api/workspace/knowledge` responds successfully, and `/api/workspace/reference-data/import` is deployed but correctly rejects the missing bundled `Import Data` folder in production. Amplify still needs a successful public release from the tracked GitHub branch with the pinned-SDK [amplify.yml](../amplify.yml), the production reference-data source must remain explicit rather than image-bundled, Aurora encryption still needs a migration plan, and the longer-term API hosting path should be reviewed beyond the current App Runner footprint.
...
- [ ] The thin API host is provisioned in AWS App Runner and the live runtime now serves snapshot and knowledge successfully, but the public Amplify release still needs to be rerun from the tracked branch and the production reference-data source remains an explicit operational decision.
...
- [ ] The live App Runner revision now serves the main workspace routes, but public Amplify cutover still needs to be rerun from the tracked branch and production reference-data bootstrap must stay on an explicit external source rather than a bundled repo folder.
- [ ] Public `api/workspace/snapshot` cutover is still incomplete from the live Amplify site until Amplify publishes a successful release against the App Runner base address using the tracked branch build.
...
- [x] Fully configured per xAI docs (<https://docs.x.ai/docs> + models page): `grok-4-1-fast-reasoning`, OpenAI-compatible `/v1/chat/completions` and `/v1/responses`, Bearer `XAI_API_KEY` auth, and direct App Runner runtime injection from AWS Secrets Manager without hardcoding.
- [x] Solid Grok portal implemented: proxy+ resource (ID gl43sm), ANY method (NONE auth), HTTP_PROXY integration to <https://api.x.ai/v1>, prod deployment (ID rd3j25). Live validation on April 23, 2026 confirmed the gateway currently returns `403 AccessDeniedException` for anonymous invoke, so App Runner production Jarvis was switched back to the direct xAI endpoint until the proxy policy is intentionally opened or signed invocation is added.
...
- [ ] Complete the Amplify release that points `WILEY_WORKSPACE_API_BASE_ADDRESS` at `https://mr7zeizxxd.us-east-2.awsapprunner.com` using the tracked branch copy of the pinned-SDK `amplify.yml`, keep recurring monthly imports on the QuickBooks panel instead of the repo-local `Import Data/` folder, and plan the Aurora encryption/App Runner successor work. Recommended Council-session baseline remains `1 vCPU / 2 GB` if the initial `0.5 vCPU / 1 GB` footprint shows startup or workload pressure.
...
**Server-side AWS Config for Wiley Widget (per query)**: Amplify app `d2ellat1y3ljd9` (us-east-2, hosting with amplify.yml node parser for Grok responses and Amplify Gen 1 Parameter Store secret `/amplify/d2ellat1y3ljd9/main/SYNCFUSION_LICENSE_KEY` for the client build-time Syncfusion license), Aurora PostgreSQL (`wiley-co-aurora-db` in private VPC with `amplify-db-schema.sql` + `AppDbContext`), Secrets Manager ("Grok" for XAI_API_KEY plus the App Runner runtime database/xAI/Syncfusion secrets), API Gateway proxy (`gl43sm` for x.ai/v1/grok-4), thin API host at `WileyCoWeb.Api` serving /api/workspace/snapshot with CORS locked to Amplify origins. All per copilot-instructions.md (env vars only, no hardcodes). Ready for `amplify publish` post-clean build.
