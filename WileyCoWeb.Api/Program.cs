...
public partial class Program
{
        ...
        Stopwatch startupStopwatch)
    {
        ...
        {
            await EnsureQuickBooksImportSchemaAsync(app.Services, logger).ConfigureAwait(false);
        }
    ...
    private static async Task EnsureQuickBooksImportSchemaAsync(IServiceProvider services, ILogger logger)
    {
        ...
        logger.LogInformation("Ensuring QuickBooks import/routing schema is present.");
        await context.Database.ExecuteSqlRawAsync(QuickBooksImportSchemaSql).ConfigureAwait(false);
        logger.LogInformation("QuickBooks import/routing schema guard completed.");
    }

    private const string QuickBooksImportSchemaSql = """
        ...
        CREATE TABLE IF NOT EXISTS quickbooks_allocation_targets (
            ...
            CONSTRAINT fk_quickbooks_allocation_targets_profiles
                FOREIGN KEY (allocation_profile_id)
                REFERENCES quickbooks_allocation_profiles(id)
                ON DELETE CASCADE
        );
        ...
        CREATE TABLE IF NOT EXISTS quickbooks_routing_rules (
            ...
            is_active BOOLEAN NOT NULL DEFAULT TRUE,
            source_file_pattern VARCHAR(200) NULL,
            default_enterprise_pattern VARCHAR(200) NULL,
            account_pattern VARCHAR(200) NULL,
            memo_pattern VARCHAR(200) NULL,
            name_pattern VARCHAR(200) NULL,
            split_account_pattern VARCHAR(200) NULL,
            target_enterprise VARCHAR(100) NULL,
            allocation_profile_id BIGINT NULL,
            CONSTRAINT fk_quickbooks_routing_rules_profiles
                FOREIGN KEY (allocation_profile_id)
                REFERENCES quickbooks_allocation_profiles(id)
                ON DELETE SET NULL
        );
        ...
        ALTER TABLE ledger_entries
            ADD COLUMN IF NOT EXISTS original_entry_scope VARCHAR(100) NULL,
            ADD COLUMN IF NOT EXISTS source_amount NUMERIC(18,2) NULL,
            ADD COLUMN IF NOT EXISTS applied_routing_rule_id BIGINT NULL,
            ADD COLUMN IF NOT EXISTS applied_routing_rule_name VARCHAR(200) NULL,
            ADD COLUMN IF NOT EXISTS applied_allocation_profile_id BIGINT NULL,
            ADD COLUMN IF NOT EXISTS applied_allocation_profile_name VARCHAR(200) NULL,
            ADD COLUMN IF NOT EXISTS routing_allocation_percent NUMERIC(18,2) NULL,
            ADD COLUMN IF NOT EXISTS routing_reason VARCHAR(500) NULL;

        CREATE INDEX IF NOT EXISTS ix_quickbooks_allocation_profiles_is_active
            ON quickbooks_allocation_profiles(is_active);

        CREATE INDEX IF NOT EXISTS ix_quickbooks_allocation_targets_allocation_profile_id
            ON quickbooks_allocation_targets(allocation_profile_id);

        CREATE INDEX IF NOT EXISTS ix_quickbooks_routing_rules_priority
            ON quickbooks_routing_rules(priority);

        CREATE INDEX IF NOT EXISTS ix_quickbooks_routing_rules_is_active
            ON quickbooks_routing_rules(is_active);

        CREATE INDEX IF NOT EXISTS ix_ledger_entries_applied_routing_rule_id
            ON ledger_entries(applied_routing_rule_id);

        CREATE INDEX IF NOT EXISTS ix_ledger_entries_applied_allocation_profile_id
            ON ledger_entries(applied_allocation_profile_id);
        """;
