...
{
  private const string UnderStatus = "Under";
    ...
    protected override void OnInitialized()
    {
        LineItems.AddRange(new[]
        {
            ...
            new BudgetLineItem("Marketing Campaigns", "Growth", "M. Reyes", 42000m, 35500m, UnderStatus),
            new BudgetLineItem("Sales Travel", "Sales", "S. Bennett", 12000m, 9800m, UnderStatus),
            new BudgetLineItem("Operations", "Finance", "D. Brown", 21000m, 18850m, UnderStatus),
            new BudgetLineItem("Legal & Compliance", "Legal", "R. Alvarez", 9000m, 7600m, UnderStatus),
...
""");
    ...
    private string BuildWorksheetXml()
    {
        ...
        {
            rows.Add(BuildMixedRow(rowIndex++, item));
        }

          rows.Add(BuildTotalRow(rowIndex));
...
""";
    ...
    private static string BuildMixedRow(int rowNumber, BudgetLineItem item)
    {
        return $"<row r=\"{rowNumber}\">" +
           $"<c r=\"A{rowNumber}\" t=\"inlineStr\"><is><t>{EscapeXml(item.Category)}</t></is></c>" +
           $"<c r=\"B{rowNumber}\" t=\"inlineStr\"><is><t>{EscapeXml(item.Department)}</t></is></c>" +
           $"<c r=\"C{rowNumber}\" t=\"inlineStr\"><is><t>{EscapeXml(item.Owner)}</t></is></c>" +
           $"<c r=\"D{rowNumber}\"><v>{item.Budget.ToString(CultureInfo.InvariantCulture)}</v></c>" +
           $"<c r=\"E{rowNumber}\"><v>{item.Actual.ToString(CultureInfo.InvariantCulture)}</v></c>" +
           $"<c r=\"F{rowNumber}\"><v>{item.Variance.ToString(CultureInfo.InvariantCulture)}</v></c>" +
           $"<c r=\"G{rowNumber}\" t=\"inlineStr\"><is><t>{EscapeXml(item.Status)}</t></is></c>" +
               "</row>";
    }

    private string BuildTotalRow(int rowNumber)
    {
      var status = Remaining >= 0 ? "Under Budget" : "Over Budget";

      return $"<row r=\"{rowNumber}\">" +
           $"<c r=\"A{rowNumber}\" t=\"inlineStr\"><is><t>Total</t></is></c>" +
           $"<c r=\"B{rowNumber}\" t=\"inlineStr\"><is><t></t></is></c>" +
           $"<c r=\"C{rowNumber}\" t=\"inlineStr\"><is><t></t></is></c>" +
           $"<c r=\"D{rowNumber}\"><v>{TotalBudget.ToString(CultureInfo.InvariantCulture)}</v></c>" +
           $"<c r=\"E{rowNumber}\"><v>{ActualSpend.ToString(CultureInfo.InvariantCulture)}</v></c>" +
